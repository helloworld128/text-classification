...
parser.add_argument('-embed-dim', type=int, default=80, help='number of embedding dimension [default: 300]')
...
tmp0 = torch.load('data/train.data')
tmp1 = torch.load('data/test.data')
...
if args.model == 'cnn':
    ...
    rnnModel = rnn.RNN('LSTM', args.embed_dim, 150, 8, 2, True, 0.3)
    train_data = DataLoader(tmp0, batch_size=args.batch_size, shuffle=True, num_workers=config.num_workers)
    test_data = DataLoader(tmp1, batch_size=args.batch_size, shuffle=True, num_workers=config.num_workers)
    try:
